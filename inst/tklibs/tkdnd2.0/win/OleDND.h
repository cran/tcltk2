...
class TkDND_DropTarget: public IDropTarget {
  private:
    ...
    const char * FormatName(UINT cfFormat) {
      ...
      GetClipboardFormatName((CLIPFORMAT) cfFormat, tmpStr, 70);
      return tmpStr;
    }; /* FormatName */
  ...
  public:
    ...
    STDMETHODIMP DragEnter(IDataObject *pDataObject, DWORD grfKeyState, 
      ...
      Tcl_Obj *typelist    = Tcl_NewListObj(0, NULL), *element,
              *actionlist  = Tcl_NewListObj(0, NULL), *objv[7],
    ...
    STDMETHODIMP Drop(IDataObject *pDataObject, DWORD grfKeyState, 
                      POINTL pt, DWORD *pdwEffect) {
      Tcl_Obj *objv[7], *result, **typeObj, *data, *type,
              *pressedkeys = Tcl_NewListObj(0, NULL);
      int i, status, index, typeObjc;
      ...
      *pdwEffect = DROPEFFECT_NONE;
      ...
      // Try to get the data.
      for (i = 0; i < typeObjc; ++i) {
        status = Tcl_GetIndexFromObj(interp, typeObj[i],
        ...
        if (data != NULL) {
          type = typeObj[i]; Tcl_IncrRefCount(type);
          break; // We have got the data!
        }
      }
      Tcl_DecrRefCount(result);
      if (data == NULL) return S_OK; // We failed to get the data.
