...
proc ::dnd {method window args} {
  switch $method {
    ...
    bindsource {
      return {}
      #return [eval tkdnd::compat::bindsource $window $args]
    }
    clearsource {
...
proc compat::bindsource2 {window type event} {
  switch $event {
    <DragEnter> {return [bind $window <<DropEnter>>]}
    <Drag>      {return [bind $window <<DropPosition>>]}
    <DragLeave> {return [bind $window <<DropLeave>>]}
...
proc compat::bindsource3 {window type event script} {
  drop_target register $window [list $type]
  switch $event {
    <DragEnter> {return [bind $window <<DropEnter>> $script]}
    <Drag>      {return [bind $window <<DropPosition>> $script]}
    <DragLeave> {return [bind $window <<DropLeave>> $script]}
...
proc compat::bindsource4 {window type event script priority} {
  return [bindsource3 $window $type $event $script]
};# compat::bindsource4
